...
## Discussion
이번 분석에서는 역세권 공동주택 데이터를 주택 유형에 따라 나누어 EDA를 수행한 후, 다세대주택/오피스텔 데이터에 대해 상관분석 및 선형회귀분석, 스플라인 회귀 분석을 수행함.
이번 분석을 통해, 전용면적당 건물 가격(매매가, 전세 보증금, 월세)은 건축년도에 큰 영향을 받는다는 것을 확인하였음. 
또한, 역세권 주택 가격 분석을 다룬 선행 연구들(이재명(2015) 등)에서 인근역까지의 거리와 건물 가격 사이에 음의 계수를 갖는 이차함수 형태의 관계가 있음을 제시했는데, 이러한 아이디어를 스플라인 회귀 분석을 통해 어느정도 확인할 수 있었음.
다만, 스플라인 회귀 분석 결과가 통계적으로 유의미하지 않은 부분이 있어 완벽한 검증은 수행하지 못함. 더불어, 선행 연구들은 역을 지상역과 지하역으로 나누거나 역의 기능별로 구분을 달리 하여 분석하였으나, 이번 분석에서는 그러한 구분이 없었기 때문에 다른 결과가 나왔을 수 있음.
이번 분석을 통해 역세권 공동주택의 전용면적당 건물 가격에 영향을 주는 변수들을 확인하고 인근역까지의 거리와 전용면적당 건물 가격 사이의 관계를 확인함으로써 역세권 공동주택 가격의 특성을 파악하였음. 이는 추후 역세권 개발에서 참고 지표로써 활용 가치가 있음.
추후 분석에서는, 역을 다양하게 세분화하여 각 그룹별로 분석해봄으로써 또다른 유의미한 결과를 얻을 수 있을 것으로 기대됨.
